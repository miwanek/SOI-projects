...
{
        message m = mm_in;
        return _taskcall(SYSTASK, SYS_GETLOT, &m); /* musimy wywołać funkcję na poziomie mikrojądra */
}
...
{
        message m = mm_in;
        return _taskcall(SYSTASK, SYS_SETLOT, &m); /* musimy wywołać funkcję na poziomie mikrojądra */
}
/*===========================================================================*
 *				showlot					     *
 *===========================================================================*/
PUBLIC void do_showlot()
{
	int current_pid;
        int procnr;
	int ticket_number;
	...
	{
   	  current_pid= mproc[procnr].mp_pid;
	  if( (mproc[procnr].mp_flags & IN_USE)!=0 && current_pid >0 ) 
		{
		   msg.m1_i1= current_pid;
		   ticket_number= _taskcall(SYSTASK, SYS_GETLOT, &msg);
		   printf("Liczba losow procesu o PID %d to %d \n", current_pid, ticket_number);
