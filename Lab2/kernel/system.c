...
/*==========================================================================*
 *				find_user_proc_by_pid			    *
 *==========================================================================*/

struct proc * find_user_proc_by_pid(int pid) /* zwracamy wskazanie na element w strukturze PROC o zadanym PID  */
{
	int I;
	for(I = NR_TASKS; I < NR_TASKS + NR_PROCS; I++) /* szukamy wyłącznie procesów user, procesy mikrojądra/serwerów nas nie interesują */
	{
		if( proc[I].p_priority != PPRI_NONE && proc[I].p_pid == pid )
			return &proc[I]; /* przy sukcesie zwracamy wskazanie na proces */
	}
	return 0; /* w przeciwnym wypadku pusty wskaźnik */
}
...
message *m_ptr;
{
	...
	if((rp = find_user_proc_by_pid(m_ptr->m1_i1)) != NIL_PROC) /* jeśli znajdziemy proces o danym PID to zwracamy aktualną liczbę losów */
		return rp->current_tickets; 
	return EINVAL; /* w przeciwnym wypadku wysyłamy błąd */
}

/*==========================================================================*
 *				do_setlot			    	    *
 *==========================================================================*/			
PRIVATE int do_setlot(m_ptr) /* funkcja ustawia liczbę losów  procesu */ 
message * m_ptr;
{
	struct proc * rp;
	...
	if((rp = find_user_proc_by_pid(m_ptr->m1_i1)) == NIL_PROC)
		return EINVAL; /* proces nie został znaleziony */
	
	if(new_tickets < 1 || new_tickets > 20)
		return EINVAL; /* niepoprawna liczba losów */

	/* Jeśli proces jest gotowy, to zmieni się jego pozycja na liście oraz liczba ticketów procesów gotowych.  */	
	if(is_ready(rp))
	{
		lock_unready(rp);
		rp->current_tickets = new_tickets;
		lock_ready(rp);	
		return 2; /* liczba losów została prawidłowo zmieniona, ale zaszła kolejność sortowania listy procesów gotowych */
	}
	
	rp->current_tickets = new_tickets;	
	return 1; 	/* liczba losów została prawidłowo zmieniona, nie trzeba sortować listy procesóœ gotowych */
}
