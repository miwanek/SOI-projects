...
unsigned short all_tickets= 0 ; /* suma losów wszystkich procesów */

/*long check_usage = 0; */ /* zmienna umożliwiająca nam syfu na ekranie */
 ...
 *				is_ready				     * 
 *===========================================================================*/
PUBLIC unsigned short is_ready(rp)
struct proc * rp;
{
	struct proc * pom_proc;
	pom_proc = rdy_head[USER_Q];
	while(pom_proc != NIL_PROC)
	{
		if(pom_proc == rp)
			return 1; /* proces jest gotowy */
		pom_proc = pom_proc->p_nextready;	
	}
	return 0; /* proces nie jest gotowy */
}
 ...
 */
  ...
  unsigned short ticket_number; /* zmienna trzyma wylosowany przez nas numer losu */
  ...
  if(all_tickets > 0) /* jeśli suma losów jest równa 0 to nie mamy żadnego procesu */
  {
	  /* za sumę losów odpowiadają ready i unready*/
	  ticket_number = (rand() % all_tickets) + 1; /* Losujemy liczbę z zakresu od 1 do all_tickets i wybieramy proces, */
	  rp = rdy_head[USER_Q];
	  while(ticket_number > rp->current_tickets)
	  {
		  ticket_number -= rp->current_tickets;
		  rp = rp->p_nextready;
	  }
	  /*if( check_usage == 100)
	  {
	  	printf("Wybrano proces o PID: %d i liczbie losow %d \n", rp->p_pid, rp->current_tickets);
		check_usage=0;
	  }
	  ++check_usage; */
 ...
 */
  ...
  /* Szukamy miejsca, gdzie wstawić proces, aby mieć porządek nierosnący */

  all_tickets += rp->current_tickets; /* dodajemy losy procesu do puli */

  /* Sprawdzamy czy jest w ogóle jakiś gotowy proces w kolejce */
  ...
  /* Sprawdzamy czy ma więcej losów od pierwszego na liście */
  if(rp->current_tickets > rdy_head[USER_Q]->current_tickets)
  {
	rp->p_nextready = rdy_head[USER_Q];
	rdy_head[USER_Q] = rp;
	return;
  }
  /* Sprawdzamy, gdzie wstawić proces */
  xp = rdy_head[USER_Q];
  while(xp->p_nextready && xp->p_nextready->current_tickets > rp->current_tickets)
  {
	xp = xp->p_nextready;
  }
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
	...
	qtail = &rdy_tail[SERVER_Q];
		...
		all_tickets -= rp->current_tickets; /* pomniejszamy sumę losów o losy procesu */ 

	  	if(rp == rdy_head[USER_Q]) /* jeśli usuwamy pierwszy proces z kolejki to następny staje się głową kolejki */
		{
			rdy_head[USER_Q] = rp->p_nextready;
		}
		else
		{
			/* Musimy załatać dziurę w kolejce */
			xp = rdy_head[USER_Q];
			while(xp && xp->p_nextready != rp)
			{
				xp = xp->p_nextready; /* przesuwamy się dopóki nie znajdziemy miejsca usuwanego procesu */
			}
			xp->p_nextready = rp->p_nextready; /* aktualizujemy wskaźniki */
		}

		if(rp == proc_ptr) pick_proc();  /* Jeśli to był aktywny proces to musimy wybrać inny */ 

		return;
  }

  /* Stara niepotrzebna obsługa procesów użytkownika */
 ...
 */
  ...
  /* Stara obsługa szeregowania procesów, wszystko co jest niezbędne robimy w ready/unready */
